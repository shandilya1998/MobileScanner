...
def run(
    ...
    elif architecture == 'yolov2_depthwise':
        model = get_depthwise_yolov2_model(plot_model)
    else:
    ...
    train_dataset = None
    train_dataset= get_dataset(train_image_folder, train_annot_folder, LABELS, BATCH_SIZE, compute = compute, scale = scale)
    ...
    def generator():
        ...
        for i in tqdm(range(scale)):
            ...
            for item in augmentation_generator(train_dataset, seed):
                yield item
    ...
    train_gen = ground_truth_generator(generator())
    ...
    def display(file, model, score_threshold, iou_threshold, step = EPOCHS):
        ...
        for i in range(count_detected):
            ...
            y = box[0]*IMAGE_H
            x = box[1]*IMAGE_W
            h = (box[2] - box[0])*IMAGE_H
            w = (box[3] - box[1])*IMAGE_W
    ...
    # batch
    img, detector_mask, matching_true_boxes, matching_true_texts, class_one_hot, true_boxes, true_texts = next(train_gen)
    ...
    # get batch
    img, detector_mask, matching_true_boxes, matching_true_texts, class_one_hot, true_boxes_grid, true_texts_grid = next(train_gen)
    ...
    true_boxes_grid = true_boxes_grid[0:1]
    ...
    # log (tensorboard)
    def log_loss(loss, val_loss, val_sub_loss, train_sub_loss, step, lr):
    ...
    def train(epochs, model, train_dataset, val_dataset, steps_per_epoch_train, steps_per_epoch_val, train_name = 'train'):
        ...
        optimizer = tf.keras.optimizers.Adam(learning_rate=lr_schedule, beta_1=beta1, beta_2=beta2, epsilon=eps)
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx in range(steps_per_epoch_train):
                img, detector_mask, matching_true_boxes, matching_true_texts, class_one_hot, true_boxes, true_texts =  next(train_dataset)
            ...
            for batch_idx in range(steps_per_epoch_val):
                img, detector_mask, matching_true_boxes, matching_true_texts, class_one_hot, true_boxes, true_texts =  next(val_dataset)
            ...
            log_loss(loss_avg, val_loss_avg, val_sub_loss_avg, train_sub_loss_avg, epoch, current_learning_rate)
...
def create_model(obj_det):
    inp = tf.keras.layers.Input((IMAGE_H, IMAGE_W, NUM_C), dtype = 'float32')
    out1, features1 = obj_det(inp)
    
    feature_extractor = get_mobilenet_model(False)

    _, features2 = feature_extractor(inp)
    ...
    x = tf.keras.layers.BatchNormalization(name='norm_ocr_1')(x)
    ...
    x = tf.keras.layers.BatchNormalization(name='norm_ocr_2')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    x = tf.keras.layers.Conv2D(
        BOX*MAX_TEXT_LENGTH*len(i2c),
        (3, 3),
        use_bias=False,
        padding='same',
        strides=(1, 1),
    ...
    x = tf.keras.layers.Reshape((GRID_H, GRID_W, BOX, MAX_TEXT_LENGTH, len(i2c)))(x)
    out2 = tf.keras.layers.Softmax(
        axis=-1
    )(x)
...
def run_combined(
    ...
    if plot_model:
        tf.keras.utils.plot_model(model, to_file='yolov2_ocr_combined_model.png', show_shapes=True)
