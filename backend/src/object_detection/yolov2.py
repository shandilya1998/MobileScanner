...
# Custom Keras layer
    ...
    # Layer 2
    x = tf.keras.layers.Conv2D(32, (3,3), strides=(1,1), padding='same', name='conv_2', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_2')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)

    # Layer 3
    x = tf.keras.layers.Conv2D(64, (3,3), strides=(1,1), padding='same', name='conv_3', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_3')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 4
    x = tf.keras.layers.Conv2D(32, (1,1), strides=(1,1), padding='same', name='conv_4', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_4')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 5
    x = tf.keras.layers.Conv2D(64, (3,3), strides=(1,1), padding='same', name='conv_5', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_5')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)

    # Layer 6
    x = tf.keras.layers.Conv2D(128, (3,3), strides=(1,1), padding='same', name='conv_6', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_6')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 7
    x = tf.keras.layers.Conv2D(64, (1,1), strides=(1,1), padding='same', name='conv_7', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_7')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 8
    x = tf.keras.layers.Conv2D(128, (3,3), strides=(1,1), padding='same', name='conv_8', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_8')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)
    x = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))(x)

    # Layer 9
    x = tf.keras.layers.Conv2D(256, (3,3), strides=(1,1), padding='same', name='conv_9', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_9')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 10
    x = tf.keras.layers.Conv2D(64, (1,1), strides=(1,1), padding='same', name='conv_10', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_10')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 11
    x = tf.keras.layers.Conv2D(256, (3,3), strides=(1,1), padding='same', name='conv_11', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_11')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 12
    x = tf.keras.layers.Conv2D(128, (1,1), strides=(1,1), padding='same', name='conv_12', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_12')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 13
    x = tf.keras.layers.Conv2D(256, (3,3), strides=(1,1), padding='same', name='conv_13', use_bias=False)(x)
    ...
    # Layer 14
    x = tf.keras.layers.Conv2D(512, (3,3), strides=(1,1), padding='same', name='conv_14', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_14')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 15
    x = tf.keras.layers.Conv2D(256, (1,1), strides=(1,1), padding='same', name='conv_15', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_15')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 16
    x = tf.keras.layers.Conv2D(512, (3,3), strides=(1,1), padding='same', name='conv_16', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_16')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 17
    x = tf.keras.layers.Conv2D(256, (1,1), strides=(1,1), padding='same', name='conv_17', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_17')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 18
    x = tf.keras.layers.Conv2D(512, (3,3), strides=(1,1), padding='same', name='conv_18', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_18')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 19
    x = tf.keras.layers.Conv2D(512, (3,3), strides=(1,1), padding='same', name='conv_19', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_19')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 20
    x = tf.keras.layers.Conv2D(512, (3,3), strides=(1,1), padding='same', name='conv_20', use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name='norm_20')(x)
    x = tf.keras.layers.LeakyReLU(alpha=0.1)(x)

    # Layer 21
    skip_connection = tf.keras.layers.Conv2D(32, (1,1), strides=(1,1), padding='same', name='conv_21', use_bias=False)(skip_connection)
    ...
    # Layer 22
    x = tf.keras.layers.Conv2D(512, (3,3), strides=(1,1), padding='same', name='conv_22', use_bias=False)(x)
