...
def get_axis_coords(coords):
    axis_coords = []
    for axis_coord in coords.split(' '):
        if float(axis_coord).is_integer():
            axis_coords.append(round(float(axis_coord)))
        else:
            axis_coords.append(round(float(axis_coord)*10000))
    return axis_coords
...
def get_all_traces(doc_namespace, root):
    ...
    for trace_tag in root.findall('trace'):
        ...
        for coord in (trace_tag.text).replace('\n', '').split(','):
            if coord.startswith(' '):
                coords.append(get_axis_coords(coord[1:]))
            else:
                coords.append(get_axis_coords(coord))
...
def get_traces_data(inkml_file_abs_path):
    ...
    traces_all.sort(key=lambda trace_dict: trace_dict['id'])
...
def inkml2img(input_path, output_path):
    ...
    for elem in traces:
        ...
        for subls in ls:
            data = np.array(subls)
            x, y = zip(*data)
            axes.plot(x, y, linewidth=2, c='black')
